...
abs_path = r"D:\challenge\data\myvocdataset\VOCData"
...
for image_set in sets:
    ...
    image_ids = open(abs_path + '/ImageSets/%s.txt' % (image_set)).read().strip().split()
    list_file = open(abs_path + '/%s.txt' % (image_set), 'w')
    for image_id in tqdm(range(len(image_ids))):
        list_file.write(abs_path + '/images/%s.jpg\n' % (image_ids[image_id]))
        convert_annotation(image_ids[image_id])
    list_file.close()
